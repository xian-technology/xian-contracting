        ...
        if event == "line":
            ...
            if new_memory_usage > self.last_frame_mem_usage:
                self.total_mem_usage += (
                    new_memory_usage - self.last_frame_mem_usage
                )
            ...
            if self.total_mem_usage > 500 * 1024 * 1024:
                self.stop()
                raise AssertionError(
                    f"Transaction exceeded memory usage! Total usage: {self.total_mem_usage} bytes"
                )

            # Add cost based on opcode
            opcode_cost = cu_costs.get(
                opcode, 1
            )  # Default cost if opcode not found
